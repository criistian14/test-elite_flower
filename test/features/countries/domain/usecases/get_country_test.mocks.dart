// Mocks generated by Mockito 5.0.13 from annotations
// in testeliteflower/test/features/countries/domain/usecases/get_country_test.dart.
// Do not manually edit this file.

import 'dart:async' as _i4;

import 'package:dartz/dartz.dart' as _i2;
import 'package:mockito/mockito.dart' as _i1;
import 'package:testeliteflower/src/core/errors/failures.dart' as _i5;
import 'package:testeliteflower/src/features/countries/domain/entities/country.dart'
    as _i6;
import 'package:testeliteflower/src/features/countries/domain/repositories/country_repository.dart'
    as _i3;

// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis

class _FakeEither<L, R> extends _i1.Fake implements _i2.Either<L, R> {}

/// A class which mocks [CountryRepository].
///
/// See the documentation for Mockito's code generation for more information.
class MockCountryRepository extends _i1.Mock implements _i3.CountryRepository {
  MockCountryRepository() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<_i2.Either<_i5.Failure, _i6.Country?>> getCountry(String? code) =>
      (super.noSuchMethod(Invocation.method(#getCountry, [code]),
              returnValue: Future<_i2.Either<_i5.Failure, _i6.Country?>>.value(
                  _FakeEither<_i5.Failure, _i6.Country?>()))
          as _i4.Future<_i2.Either<_i5.Failure, _i6.Country?>>);
  @override
  _i4.Future<_i2.Either<_i5.Failure, void>> saveCountry(_i6.Country? country) =>
      (super.noSuchMethod(Invocation.method(#saveCountry, [country]),
              returnValue: Future<_i2.Either<_i5.Failure, void>>.value(
                  _FakeEither<_i5.Failure, void>()))
          as _i4.Future<_i2.Either<_i5.Failure, void>>);
  @override
  String toString() => super.toString();
}
